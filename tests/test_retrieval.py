...
def test_retriever_normal_mode_blocks_poison():
    ...
    payload = retriever.retrieve(
        brief_text=(
            "Social media launch post teaser "
            "for Aurora X3"
        ),
    ...
    assert all(
        not chunk.poisoned
        for chunk in payload.similar_examples
    )

    assert all(
        not chunk.poisoned
        for chunk in payload.style_rules
    )


def test_retriever_returns_only_approved_examples():
    retriever = KBRetriever()
    ...
    assert all(
        example.approved
        for example in payload.similar_examples
    )
