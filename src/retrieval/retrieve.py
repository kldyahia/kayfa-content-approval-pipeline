...
class KBRetriever:
    ...
    def _filters(
        ...
        # Only approved examples should be retrieved
        # as reference examples.
        if doc_type == "approved_example":
            filters.append(
                MetadataFilter(
                    key="approved",
                    value="true",
    ...
    def retrieve(
        ...
        if exclude_poisoned:
            ...
            excluded_count = sum(
                ...
                if node.node.metadata.get("poisoned") == "true"
            )
        ...
        def to_chunk(node) -> RetrievedChunk:
            ...
            return RetrievedChunk(
                ...
                poisoned=(
                    metadata.get("poisoned") == "true"
                ),
                approved=(
                    metadata.get("approved", "true")
                    == "true"
                ),
            )
